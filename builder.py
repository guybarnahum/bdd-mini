...
# --- ARGUMENT PARSING (Config Path and Exclusive Split) ---
    ...
    with zipfile.ZipFile(labels_zip, 'r') as z_lbl:
        ...
        for f in all_json:
            ...
            selected_videos.append({
                "name": v_name, "frames": raw_data, "source_type": "bdd", "split": split, "step": step
            })
            pbar.update(1)
        pbar.close()
            ...
            with RemoteZip(actual_url) as z_dt:
                ...
                for gt_f in tqdm(seq_gt_files, desc="DT Scan"):
                    ...
                    selected_videos.append({
                        ...
                        "source_type": "dancetrack", "zip_url": zip_url, "split": split, "step": step
                    })
        except Exception as e: 
    ...
    with zipfile.ZipFile(lbl_zip_path, 'r') as z_vis:
        ...
        for f in tqdm(txt_files, desc="VisDrone Scan"):
            ...
            selected_videos.append({
                ...
                "source_type": "visdrone", "split": split, "step": step
            })
...
def build_mini_dataset(target_split=None, video_limit=20):
    ...
    if need_bdd: all_videos.extend(select_bdd_videos(cfg.get('bdd', {}), labels_zip))
    if need_dancetrack: all_videos.extend(select_dancetrack_videos(cfg.get('dancetrack', {})))
    if need_visdrone: all_videos.extend(select_visdrone_videos(cfg.get('visdrone', {})))
    ...
    if target_split:
        ...
        for v in all_videos:
            ...
            elif v['split'] == 'test': test_set.append(v)
            else: train_pool.append(v)
        ...
        # A. Process Source-Specific Budgets First with STRICT Slicing
        for source_name, candidates in train_candidates.items():
            ...
            if specific_budget is not None:
                ...
                for v in candidates:
                    ...
                    needed = specific_budget - source_collected
                    raw_frames = sorted(v['frames'], key=lambda x: x.get('frameIndex', 0))
                    # Keeps original frames up until the budget is met
                    v['frames'] = raw_frames[:needed * v['step']]
                    effective_len = len([x for i, x in enumerate(v['frames']) if i % v['step'] == 0])
                    train_set.append(v)
                    added_video_names.add(v['name'])
                    source_collected += effective_len
                    collected_frames += effective_len

        # B. Process Global Budget with STRICT Slicing
        if global_budget is not None:
            ...
            for v in remaining_candidates:
                ...
                needed = global_budget - collected_frames
                raw_frames = sorted(v['frames'], key=lambda x: x.get('frameIndex', 0))
                v['frames'] = raw_frames[:needed * v['step']]
                effective_len = len([x for i, x in enumerate(v['frames']) if i % v['step'] == 0])
                train_set.append(v)
    ...
    if need_visdrone: download_queue.append({"type": "visdrone", "opener": zipfile.ZipFile, "path": cfg['visdrone']['images_zip']})
    if need_dancetrack:
            ...
            with source['opener'](source['path']) as z:
                ...
                for filename in all_files:
                    ...
                    for v_name in possible_videos:
                        if v_name in path_parts: found_video = v_name; break
                ...
                for file_path, split, v_name in tqdm(files_to_process, desc=f"Extracting {source['type']}"):
                    try:
                        ...
                        # FINAL GUARD: Check if frame index is within the sliced range for that video
                        if frame_idx >= len(video_source_map[v_name]['frames']): continue
...
if __name__ == "__main__":
    # Script already configured above to use 'args' from the parser
